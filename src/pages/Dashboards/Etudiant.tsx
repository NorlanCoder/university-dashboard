...
const Etudiant: React.FC = () => {
  ...
  const [studentsDetails, setProfsDetails] = useState<Member | null>(null);
  const [selectedStudents, setSelectedProfs] = useState<Member | null>(null);
  ...
  const perPage = 10; // Nombre d'étudiant par page
...
const itemVariants = {
  ...
  // Charger les students

  const loadStudents = async () => {
    setLoading(true);
    try {
      const response = await fetchMembersByRole("student", page, search);
      if (response && response.persons) {
        const validProfs = response.persons.filter((student) => !student.is_delete);
        setStudents(validProfs);
  ...
  const toggleAddEditDialog = (students: Member | null = null) => {
    setSelectedProfs(students); // Définit l'admin sélectionné ou réinitialise à null
  ...
  const handleSubmit = async (values: StudentFormValues,
    ...
    try {
      if (selectedStudents) {
        // Modification
        await updateMember(selectedStudents.id, values.name, "student", values.phone, values.email);
        toast.success("Etudiant mis à jour avec succès.");
      } else {
        // Ajout
        await addMember(values.name, "student", values.phone, values.email);
  ...
  return (
    <>
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink className="text-xl">Étudiants</BreadcrumbLink>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      >
        ...
        <motion.div  variants={itemVariants} className=" flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
            <ImportExcelMembers role="student" onImportSuccess={loadStudents} />
          ...
          <TableBody>
            {loading ? (
              ...
              students.map((student) => (
                <TableRow key={student.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 hover:shadow-sm divide-x divide-gray-300 dark:divide-gray-700">
                  <TableCell className="p-3 text-left dark:text-gray-300">{student.matricule}</TableCell>
                  <TableCell className="p-3 text-left dark:text-gray-300">{student.name}</TableCell>
                  <TableCell className="p-3 text-left dark:text-gray-300">{student.email}</TableCell>
                  <TableCell className="p-3 text-left dark:text-gray-300">{student.phone}</TableCell>
                  <TableCell className="p-3 text-center flex justify-center items-center">
                    <Button className="mr-2" size="sm" variant="primary" onClick={() => toggleAddEditDialog(student)}>
                      <PencilSquareIcon className="h-5 w-5 mx-auto" />
                    </Button>
                    <Button size="sm" variant="default" onClick={() => handleViewDetails(student.id)}>
      ...
      <Dialog open={isAddEditDialogOpen} onOpenChange={setIsAddEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{selectedStudents ? "Modifier" : "Ajouter"} un étudiant</DialogTitle>
          </DialogHeader>
          <Formik<StudentFormValues>
            initialValues={{
              name: selectedStudents?.name || "",
              email: selectedStudents?.email || "",
              phone: selectedStudents?.phone || "",
            }}
            validationSchema={Yup.object({
              name: Yup.string().matches(/^[A-Za-zÀ-ÖØ-öø-ÿ\s'-]+$/, "Le nom ne doit contenir que des lettres.").required("Nom requis"),
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={() => setIsDetailsDialogOpen(false)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Détails Etudiant</DialogTitle>
          </DialogHeader>
          {studentsDetails ? (
            <div>
              <p className="mb-2"><strong>Matricule :</strong> {studentsDetails.matricule}</p>
              <p className="mb-2"><strong>Nom :</strong> {studentsDetails.name}</p>
              <p className="mb-2"><strong>Email :</strong> {studentsDetails.email}</p>
              <p><strong>Téléphone :</strong>  {studentsDetails.phone}</p>
            </div>
          ) : (
