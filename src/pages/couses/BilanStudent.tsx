...
import { fetchListBilan, BilanData } from '@/api/Classroom/BilanStudent';
import { fetchListEtudiant, ClassInfo } from '@/api/Classroom/ListStuduents';
import { Card, CardHeader, CardContent, CardFooter } from '@/components/ui/card';
import {Table,TableBody,TableCell,TableHeader,  TableRow, } from '@/components/ui/table';
import { toast } from 'react-hot-toast';
import { motion, AnimatePresence } from 'framer-motion';
const BilanStudent: React.FC = () => {
  const { classId, bilan } = useParams<{ classId: string; bilan: string }>();
  ...
  // Charger les informations sur la classe
  const loadClassInfo = async () => {
   ...
   try {
     const response = await fetchListEtudiant(classId);
     const firstClass = response.bilans[0]?.class; 
     setClassInfo(firstClass || null);
   } catch (error) {
     toast.error('Erreur lors du chargement des informations sur la classe.');
     console.error(error);
   }
 };
 ...
 useEffect(() => {
  ...
  const loadBilan = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetchListBilan(classId, bilan);
  ...
  const { student, general } = bilanData;
...
<div className="p-6">
      ...
      <div className=" dark:bg-orange-400 bg-blue-700 mb-6 p-4 border shadow-md rounded-lg flex justify-between">
        ...
        <div>
          {classInfo ? (
            ...
            <p className="text-center text-white italic">En cours</p>
          )}
        </div>
      </div>
  ...
  <Card key={semester.sem} className="mb-6 shadow-lg  dark:bg-gray-800 dark:border-gray-700">
    <CardHeader
